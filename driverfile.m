%Jonathan Cushman - Driver File

%Special Instructions 
%Subroutine 1 -> use routine 4 for column oriented Lv mult and in routine 4
%uncomment the function call

%Subroutine 2 -> use routine 4 for compressed column oriednted
%multiplication and uncomment subroutine 2 function call

%subroutine 3-> use routine 4 and uncomment subroutine 3 function call AND
%make sure to uncomment line 220 to 235

%subroutine 4 -> use routine 2 and thats it, the function is already
%uncommented so nothing to change and it doesnt mess with the other stuff 

%Currently set to set subroutine 4
...
%
  ...
  elseif (routinenum == 3) || (routinenum == 4)
    ...
    %%%Comment this section out when testing subroutine 1 and 2
    %%% Only needed for testing subroutine 3
        if routinenum == 4
    % Replace L with banded version
        L = eye(n);
           for i = 2:n
           L(i,i-1) = Temp(i,i-1); % first subdiagonal
          end

            for i = 3:n
            L(i,i-2) = Temp(i,i-2); % second subdiagonal
            end
...
%
    maxrelerror = max(maxrelerror,RelError(itest));
  elseif (routinenum == 2) 
    [Acomp] = LUmult(M,n);   %edited from origional call 
    ErrorMatrix = Acomp(1:n,1:n) - Atrue(1:n,1:n);
%
...
%subroutine number 1:
    %[zcomp] = Lvmult_col(M,v,n);

%subroutine number 2: 
    %[zcomp] = Lvmult_col_compressed(M,v,n);

%subroutine number 3: 
    [zcomp] = Lvmult_col_banded(M,v,n);
